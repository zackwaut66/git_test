// Deterministic-ish Monte Carlo balance smoke test for Prototype 0.1.
// This mirrors the current formation combat closely enough to catch impossible encounters
// and extreme tuning regressions without requiring a browser.
...
function party(region){
  // Model modest natural progression by deeper regions: levels/gear earned on the way in.
  const bonus=[0,3,7,11][region];
  return [
    ...
    {name:'Enclave Guard',hp:105,max:105,atk:11,row:'front',status:{}},
    {name:'March Scout',hp:78,max:78,atk:14,row:'back',status:{}}
  ];
}
...
function run(regionIndex){
  ...
  while(living(p).length&&living(es).length&&turn<120){
    ...
    // Simple player policy: emergency heal, then sever, then focus; otherwise save Resolve.
...
for(let r=0;r<regions.length;r++){
  ...
  const rate=wins/RUNS_PER_REGION;
  total+=RUNS_PER_REGION;
  ...
  if(rate<.08||rate>.995)throw new Error(`${regions[r].name} is outside broad playable balance bounds: ${rate}`);
}
