...
function stats(h,s){const gear=gearFor(h.name,s);let atk=(h.atk||0)+((h.lv||1)-1)*3,hp=(h.hp||0)+((h.lv||1)-1)*8;for(const x of gear){atk+=x.atk||0;hp+=x.hp||0}const pilgrim=Object.values(s.equipped||{}).filter(x=>x?.set==='Ashen Pilgrim').length,mourning=Object.values(s.equipped||{}).filter(x=>x?.set==='Mourning Watch').length;if(pilgrim>=2)atk+=4;if(mourning>=2)hp+=18;return{atk,hp,gear}}
function build(){
  ...
  view.innerHTML=`<section class="v7-hunterhall" aria-label="Hunter Hall">
    ...
    <div class="v7-hero" style="--hero:url('${ART[h.name]||ART.Vanguard}')" aria-hidden="true"></div>
    <nav class="v7-roster" aria-label="Active Hunters">${(s.hunters||[]).map(x=>`<button data-v7-hunter="${esc(x.name)}" class="${x.name===h.name?'active':''}" style="--thumb:url('${ART[x.name]||ART.Vanguard}')"><b>${esc(x.name)}</b><small>LV ${x.lv||1}</small></button>`).join('')}</nav>
