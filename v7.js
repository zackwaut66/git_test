...
if(!root)return;
const artURI=k=>{const data=window.__HART?.[k];return data?`data:image/webp;base64,${data}`:''};
const ART={Vanguard:artURI('v'),Duelist:artURI('d'),Physician:artURI('p')};
...
const artFor=name=>ART[name]||FALLBACK[name]||FALLBACK.Vanguard;
...
function build(){
  ...
  view.innerHTML=`<section class="v7-hunterhall" aria-label="Hunter Hall">
    ...
    <div class="v7-hero" data-art="generated" style="--hero:url('${artFor(h.name)}')" aria-hidden="true"></div>
