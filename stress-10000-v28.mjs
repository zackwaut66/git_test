...
const summary=await page.evaluate(()=>{
 ...
 function kingdomFor(run){
   const count=1+(run%6),plots=[];
   for(let i=0;i<6;i++){
     if(i>=count){plots.push({reclaimed:false,building:null,level:0});continue}
   ...
   kingdomCoverage.reclaimed[count-1]++;
 ...
 function validate(run,kind,k,expected){
   const s=Game.state,b=KingdomV27.bonuses();
   if(s.coin<0||s.iron<0||s.salvage<0)invariants.push(`${run}:${kind}:negative resource`);
   if(s.clears.some(x=>x<0))invariants.push(`${run}:${kind}:negative clears`);
   if(k.plots.length!==6||!k.plots[0].reclaimed)invariants.push(`${run}:${kind}:invalid land sequence`);
   let sawLock=false;for(const p of k.plots){if(!p.reclaimed)sawLock=true;else if(sawLock)invariants.push(`${run}:${kind}:nonsequential reclaimed land`);if(p.level<0||p.level>3)invariants.push(`${run}:${kind}:invalid building level`)}
   for(const key of Object.keys(expected))if(b[key]!==expected[key])invariants.push(`${run}:${kind}:bonus ${key} expected ${expected[key]} got ${b[key]}`);
   for(const [slot,item] of Object.entries(s.equipped||{}))if(!s.inventory.some(x=>x.id===item.id))invariants.push(`${run}:${kind}:missing equipped item ${slot}`);
   if(s.wardenDefeated&&s.clears[3]<1)invariants.push(`${run}:${kind}:warden flag without clear`);
   const h=s.hunters[0],hs=Game.hunterStats(h);if(hs.hp<h.hp+expected.hp||hs.atk<h.atk+expected.atk)invariants.push(`${run}:${kind}:kingdom hunter stats not applied`);
 }
 for(let run=0;run<10000;run++){
   const kind=profiles[run%profiles.length],st=stats[kind];st.runs++;
 ...
 for(const s of Object.values(stats)){s.completionRate=+(100*s.completed/s.runs).toFixed(2);s.avgAttempts=+(s.attempts/s.runs).toFixed(2);s.avgCoin=+(s.coin/s.runs).toFixed(1);s.avgIron=+(s.iron/s.runs).toFixed(1);s.avgSalvage=+(s.salvage/s.runs).toFixed(1);s.avgLevel=+(s.levels/s.runs).toFixed(2)}
 return {runs:10000,stats,regionAttempts,regionWins,invariantCount:invariants.length,invariants:invariants.slice(0,50),deadlockCount:deadlocks.length,deadlocks:deadlocks.slice(0,50),kingdomCoverage,guildRoundTrips,kingdomRoundTrips};
});
summary.browserErrors=browserErrors;
fs.writeFileSync('stress-10000-v28-results.json',JSON.stringify(summary,null,2));
