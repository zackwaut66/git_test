...
function decorate(hall){
  ...
  const panel=buildPanel(h,d);
  const stats=hall.querySelector('.v7-stats'),action=hall.querySelector('.v7-hall-action');
  if(stats)stats.insertAdjacentElement('afterend',panel);else if(action)action.insertAdjacentElement('beforebegin',panel);else hall.append(panel);
}
function scan(){ROOT.querySelectorAll('.v7-hunterhall').forEach(decorate)}
let queued=false;
